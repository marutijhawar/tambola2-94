...
export const NumberPicker = ({
  ...
  const availableNumbers = Array.from({
    length: 90
  }, (_, i) => i + 1).filter(num => !pickedNumbers.includes(num));
  const speakNumber = (number: number) => {
    ...
    const preferredVoice = voices.find(voice => voice.lang.includes('en') && (voice.name.includes('Google') || voice.name.includes('Microsoft')));
    if (preferredVoice) {
  ...
  return <div className="space-y-6">
      ...
      <Card className="p-8 text-center bg-gradient-to-br from-yellow-600 to-yellow-800 shadow-float border-4 border-black bg-yellow-400">
        <div className="space-y-4">
          ...
          <Button onClick={pickRandomNumber} disabled={isAnimating || availableNumbers.length === 0} size="lg" className="bg-black/20 hover:bg-black/30 text-black border-black border-2 backdrop-blur-sm transition-all duration-300 hover:scale-105" variant="outline">
      ...
      <Card className="p-6 bg-card shadow-card">
        <div className="flex items-center justify-between mb-4">
          ...
          <Button onClick={onClearHistory} size="sm" variant="outline" className="text-muted-foreground hover:text-destructive">
            <Trash2 className="w-4 h-4 mr-1" />
            Clear
          </Button>
        </div>

        {pickedNumbers.length === 0 ? <p className="text-muted-foreground text-center py-4">
            No numbers called yet. Start the game!
          </p> : <div className="grid grid-cols-6 sm:grid-cols-8 md:grid-cols-10 gap-2 max-h-64 overflow-y-auto">
            {pickedNumbers.map((number, index) => <Badge key={number} variant="secondary" className={`
