...
export const GameHost = () => {
  ...
  const numberNicknames: { [key: number]: string } = {
    1: "Nelson's Column", 2: "One Little Duck", 3: "Cup of Tea", 4: "Knock at the Door",
    ...
    18: "Coming of Age", 19: "Goodbye Teens", 20: "One Score", 21: "Royal Salute",
    22: "Two Little Ducks", 30: "Dirty Gertie", 33: "Two Little Fleas", 44: "Droopy Drawers",
    45: "Halfway There", 50: "Half Century", 55: "Snakes Alive", 66: "Clickety Click",
    69: "Either Way Up", 77: "Sunset Strip", 88: "Two Fat Ladies", 90: "Top of the Shop"
  };
  ...
  const checkPatterns = (player: Player): string[] => {
    const { ticket, markedNumbers } = player;
    const wins: string[] = [];
  ...
  const handlePatternClaim = (playerId: number, pattern: string) => {
    ...
    if (isValidClaim) {
      ...
      setPlayers(prevPlayers => 
        prevPlayers.map(p => 
          p.id === playerId 
            ? { ...p, patterns: { ...p.patterns, [pattern]: true } }
            : p
        )
      );

      setClaimedPatterns(prev => new Set([...prev, pattern]));
      setWinners(prev => [...prev, { pattern, player }]);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Game Controls */}
      {gameState !== 'setup' && gameReady && (
        <Card className="p-6 bg-gradient-primary shadow-float">
      ...
      {gameState !== 'waiting' && gameState !== 'setup' && gameReady && (
        <Card className="p-8 text-center bg-gradient-secondary shadow-card">
          ...
          {currentNumber && numberNicknames[currentNumber] && (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
          {players.map(player => (
            <PlayerTicket
              ...
              onPatternClaim={handlePatternClaim}
              gameState={gameState}
              currentNumber={currentNumber}
              claimedPatterns={claimedPatterns}
            />
          ))}
        </div>
      )}
      ...
      {calledNumbers.length > 0 && gameState !== 'setup' && gameReady && (
