...
export const TicketGenerator = ({ markedNumbers, onNumberClick }: TicketGeneratorProps) => {
  ...
  const generateTickets = () => {
    ...
    setTimeout(() => {
      const newTickets = Array.from({ length: numTickets }, (_, i) => i + 1);
  ...
  return (
    <div className="space-y-6">
      {/* Controls */}
      <Card className="p-6 bg-gradient-accent shadow-card">
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-end">
            <div className="flex-1 space-y-2">
              ...
              <Input
                ...
                max="12"
                value={numTickets}
                onChange={(e) => setNumTickets(Math.max(1, Math.min(12, parseInt(e.target.value) || 1)))}
            ...
            <div className="flex gap-2">
              <Button
                ...
                disabled={isGenerating}
                className="bg-white/20 hover:bg-white/30 text-accent-foreground border-white/30 backdrop-blur-sm"
                variant="outline"
              >
      ...
      {tickets.length === 0 && (
        <Card className="p-12 text-center bg-muted/50 border-dashed border-2 border-muted">
          <div className="space-y-4">
